...
def group_findings_for_reporting(findings: list[Finding]) -> list[_GroupedFinding]:
    ...
    try:
        ...
        for f in findings:
            ...
            rec_votes[key][f.recommendation] = rec_votes[key].get(f.recommendation, 0) + 1
...
def _compute_scorecard(findings: list[Finding]) -> list[tuple[str, float, str]]:
    ...
    for cat, codes in _SCORECARD_CATEGORIES.items():
        cat_findings = [f for f in findings if codes.intersection(f.waf_codes)]
        if not cat_findings:
            score = 100.0
        else:
    ...
    try:
        ...
        for pk, counts in by_pillar.items():
            ...
            if score >= 90:
                ...
                status = "High Risk"
            result.append(
                (pk.replace("_", " ").title(), score, status, total, crit, high, med, low)
            )
...
def calculate_remediation_priority(finding: Finding) -> int:
    ...
    """
    return _REMEDIATION_SEV_WEIGHT.get(finding.severity.value, 0) + _REMEDIATION_PILLAR_BONUS.get(
        finding.pillar, 0
    )
...
def _generate_recommendations(
    ...
    if critical > 0:
        ...
        action = f"Prioritise remediation of {high} High severity finding(s) within 14 days."
        rationale = (
    ...
    top_rule, top_count = max(rule_counts.items(), key=lambda x: x[1]) if rule_counts else ("—", 0)
    if top_count >= 3:
        action = (
...
def _compute_key_business_risks(
    ...
    below_threshold = [p for p, ps in agg.findings_by_pillar.items() if ps.compliance_score < 0.70]
    if below_threshold:
...
def _compute_top_5_actions(
    ...
    sorted_f = sorted(
        findings,
        key=lambda f: (
            _SEVERITY_ORDER.index(f.severity.value) if f.severity.value in _SEVERITY_ORDER else 99,
            -f.confidence_score,
        ),
    )[:5]
    return [
...
def _make_page_footer(agg: AggregatedReport) -> Callable:
    ...
    def _draw(canvas: Any, doc: Any) -> None:
        ...
        canvas.line(
            _MARGIN, _BOTTOM_MARGIN - 0.25 * cm, _PAGE_WIDTH - _MARGIN, _BOTTOM_MARGIN - 0.25 * cm
        )
        # Footer text
...
class PdfGenerator:
    ...
    def _section_cover(
        ...
        kpi_cards = [
            (
                "Overall Compliance",
                f"{agg.overall_compliance_score:.1f}%",
                _kpi_color(agg.overall_compliance_score),
            ),
            (
                "Risk Score",
                f"{agg.overall_risk_score:.1f}%",
                _kpi_color(100 - agg.overall_risk_score),
            ),
            ("Total Findings", str(agg.total_findings), _C_BLUE),
            ("Resources Assessed", str(agg.total_resources), _C_TEAL),
        ]
    ...
    def _section_executive_summary(
        ...
        if top_actions:
            ...
            action_rows = [
                ["#", "Finding Title", "Affected Resource", "Severity", "Business Impact"]
            ]
            sev_extra: list = []
        ...
        proj_rows = [
            ...
            [
                "After High Findings Remediated",
                f"{after_high_c:.1f}%",
                f"+{after_high_c - current_c:.1f}%" if after_high_c > current_c else "—",
            ],
            [
                "After High + Medium Remediated",
                f"{after_hm_c:.1f}%",
        ...
        try:
            ...
            if ps:
                ...
                mat_data = [
                    [
                        f"MATURITY LEVEL:   {maturity.upper()}     (Average Pillar Score: {avg_s:.1f} / 100)"
                    ]
                ]
    ...
    def _section_executive_dashboard(
        ...
        score_rows = [
            ...
            [
                ...
                f"{agg.weighted_severity_score:.1f}%",
                "Severity-weighted finding density",
            ],
            [
                "Business Impact Score",
                f"{agg.business_impact_score:.1f}%",
        ...
        if agg.findings_by_pillar:
            ...
            pillar_sev_data = {
                name: dict(ps.findings_by_severity) for name, ps in agg.findings_by_pillar.items()
            }
            story.append(
                build_findings_by_pillar_stacked(
                    pillar_sev_data,
    ...
    def _section_visual_dashboards(
        ...
        _dash_header(
            ...
            "Compliance score per WAF pillar (0–100). " "Red < 50 | Amber 50–69 | Green ≥ 70.",
        )
    ...
    def _section_resource_inventory(
        ...
        rt_pcts = []
        for stats in sorted(
            agg.resource_type_inventory.values(), key=lambda s: s.total, reverse=True
        ):
    ...
    def _section_resource_group_breakdown(
        ...
        if not rows:
            story.append(
                Paragraph("Not Available — resource groups could not be parsed.", st["na"])
            )
            return
        ...
        rg_pcts = []
        table_rows = [
            ["Resource Group", "Finding Resources", "Findings", "Critical", "High", "Risk Level"]
        ]
        for name, res, count, crit, high, _ in rows:
            risk_level = (
                "Critical" if crit > 0 else "High" if high > 0 else "Medium" if count > 0 else "Low"
            )
    ...
    def _section_waf_benchmark(
        ...
        pillar_scores = {
            p: round(ps.compliance_score * 100, 1) for p, ps in agg.findings_by_pillar.items()
        }
    ...
    def _section_business_impact(
        ...
        try:
            ...
            cat_extra = [
                ("BACKGROUND", (1, i), (1, i), _IMPACT_LEVEL_COLORS.get(cat_rows[i][1], _C_LGREY))
        ...
        for f in findings:
            ...
            if cat not in impact_map:
                impact_map[cat] = {}
            impact_map[cat][f.severity.value] = impact_map[cat].get(f.severity.value, 0) + 1
        ...
        try:
            _priority_findings = [f for f in findings if f.severity.value in ("critical", "high")][
                :5
            ]
        ...
        for f in findings:
            ...
            cur_idx = (
                _SEVERITY_ORDER.index(rt_risk[lbl][1]) if rt_risk[lbl][1] in _SEVERITY_ORDER else 99
            )
            new_idx = (
                _SEVERITY_ORDER.index(f.severity.value)
                if f.severity.value in _SEVERITY_ORDER
    ...
    def _section_executive_insights(
        ...
        try:
            ...
            story.append(Paragraph(_code_xml(insights.assessment_narrative, 1000), st["narrative"]))
        except Exception:
            pass
    ...
    def _section_architecture(
        ...
        hier_rows = [
            ...
            ["Subscriptions", str(agg.subscription_count or "N/A"), "Azure subscription scope"],
            [
                "Resource Groups",
                str(rg_count) if rg_count > 0 else "N/A",
                "Extracted from resource IDs in findings",
            ],
            [
                "Resource Types",
                str(rt_count) if rt_count > 0 else "N/A",
                "Distinct ARM resource type namespaces",
            ],
            ["Total Resources", str(agg.total_resources), "Resources assessed in this run"],
        ]
    ...
    def _section_waf_control_pages(
        ...
        for code in codes_sorted:
            ...
            has_critical_high = any(f.severity.value in ("critical", "high") for f in ctrl_findings)
            compliance_status = "Non-Compliant" if has_critical_high else "Partially Compliant"
            status_color = (
                colors.HexColor("#FADBD8") if has_critical_high else colors.HexColor("#FDEBD0")
            )
            ...
            if unique_resources:
                story.append(
                    Paragraph(
                        f"Affected Resources ({len(unique_resources)} unique)",
                        st["h3"],
                    )
                )
            ...
            story.append(
                Paragraph(
                    f"Associated Findings (top {min(8, len(ctrl_findings))})",
                    st["h3"],
                )
            )
    ...
    def _section_trend_analysis(
        ...
        if not agg.trend_data:
            ...
            story.append(
                Paragraph(
                    "Trend analysis unavailable. " "Historical assessments not yet available.",
                    st["na"],
                )
            )
            story.append(
    ...
    def _section_compliance_roadmap(
        ...
        extra: list = []
        for i, ((_, score), full_lbl) in enumerate(
            zip(scenarios, full_labels, strict=False), start=1
        ):
    ...
    def _section_remediation_roadmap(
        ...
        for week_label, week_findings in weeks.items():
            ...
            sev_extra = [
                ("BACKGROUND", (3, i), (3, i), _SEV_COLORS.get(r[3].lower(), _C_WHITE))
    ...
    def _section_executive_remediation_roadmap(
        ...
        for phase in phases:
            ...
            sev_extra = [
                ("BACKGROUND", (2, idx), (2, idx), _SEV_COLORS.get(r[2].lower(), _C_WHITE))
        ...
        for phase in phases:
            ...
            dominant_effort = (
                max(effort_counts, key=lambda k: effort_counts[k]) if effort_counts else "—"
            )
            summary_rows.append(
                [
                    f"{phase['name']}  ({phase['timeframe']})",
    ...
    def _section_enterprise_remediation_roadmap(
        ...
        try:
            ...
            if not plan.quick_wins:
                story.append(Paragraph("No quick wins identified in this assessment.", st["na"]))
            else:
                rows = [
                    ["#", "Finding", "Pillar", "Impact", "Effort", "WAF Controls", "Recommendation"]
                ]
            ...
            if not plan.timeline:
                ...
                for period in plan.timeline:
                    ...
                    if period.activities:
                        act_rows = [
                            [Paragraph(f"• {act}", st["body"])] for act in period.activities
                        ]
        ...
        try:
            ...
            caveat_data = [[Paragraph(f"<i>{_code_xml(ei.caveat, 400)}</i>", st["caption"])]]
        ...
        try:
            ...
            story.append(_make_table(ov_rows, col_widths=[6 * cm, 10.7 * cm], extra_style=extra_ov))
            story.append(Spacer(1, 0.4 * cm))
            ...
            if ms.top_risks:
                ...
                risk_rows = [
                    [Paragraph(f"• {_code_xml(r, 200)}", st["body"])] for r in ms.top_risks
                ]
    ...
    def _section_traceability_matrix(
        ...
        story.append(
            _make_table(
                matrix_rows,
                col_widths=[3.5 * cm, 3.5 * cm, 2.8 * cm, 2 * cm, 2.3 * cm, 1.8 * cm, 4.5 * cm],
                extra_style=extra,
            )
        )
        if len(findings) > 200:
    ...
    def _section_detailed_findings(
        ...
        for pillar_name in sorted(pillar_findings.keys()):
            ...
            for g in grouped:
                ...
                banner_data = [
                    [
                        Paragraph(
                            ...
                            ParagraphStyle(
                                "BannerLeft", parent=st["body"], textColor=text_color, fontSize=8
                            ),
                        ),
                        Paragraph(
                ...
                banner = Table(banner_data, colWidths=[_BODY_WIDTH * 0.70, _BODY_WIDTH * 0.30])
                banner.setStyle(
                    TableStyle(
                        [
                ...
                rem_table.setStyle(
                    TableStyle(
                        [
                            ...
                            ("BACKGROUND", (1, 4), (1, 6), colors.HexColor("#F0F3F4")),
                            ("BOX", (0, 0), (-1, -1), 0.4, _C_MGREY),
                    ...
                    if _ev_card is not None:
                        ...
                        conf_bar_data = [
                            [
                                ...
                                Paragraph(
                                    f"<b>{pct}%</b>",
                                    ParagraphStyle("ConfPct", parent=_val, alignment=TA_RIGHT),
                                ),
                            ]
                        ]
                        conf_bar_tbl = Table(conf_bar_data, colWidths=[_VW * 0.75, _VW * 0.25])
                        conf_bar_tbl.setStyle(
                            TableStyle(
                                [
                                    ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                        ...
                        if _ev_card.waf_controls:
                            ...
                            ctrl_tbl = Table(
                                ctrl_rows, colWidths=[2.5 * cm, _BODY_WIDTH - 2.5 * cm]
                            )
                            ctrl_tbl.setStyle(
                                TableStyle(
                                    [
                        ...
                        meta_rows = [
                            [
                                Paragraph("Business Impact", _lbl),
                                Paragraph(
                                    _code_xml(_ev_card.business_impact_text, 400),
                                    _val,
                                ),
                            ],
                            [
                            ...
                            [
                                ...
                                Paragraph(
                                    _code_xml(_ev_card.verification_step, 400),
                                    _val,
                                ),
                            ],
                        ]
    ...
    def _section_appendix(
        ...
        detail_rows = [
            ...
            ["Risk Score", "100 − compliance + (critical+high)/total × 10", "0–100"],
            ["Weighted Severity", "Σ(sev_weight × count) / total × 100", "0–100"],
            [
                "Business Impact",
    ...
    def _section_compliance_framework_mapping(
        ...
        try:
            story.append(
                Paragraph("C-3  CIS / ISO 27001 / NIST CSF / MCSB Cross-Reference", st["h2"])
            )
            story.append(
                Paragraph(
    ...
    def _section_risk_matrix(
        ...
        try:
            ...
            def _cell_color(lk: str, im: str) -> colors.Color:
                score = {"High": 3, "Medium": 2, "Low": 1}.get(lk, 1) * {
                    "Critical": 4,
                    "High": 3,
                    "Medium": 2,
                    "Low": 1,
            ...
            # Header row
            heatmap_rows = [["Likelihood \\ Impact"] + _IMPACT_LABELS]
            ...
            legend_rows = [
                ...
                ["", "Yellow", "Medium", "Moderate risk. Address within the current quarter."],
                [
                    "",
                    "Green",
                    "Low",
        ...
        try:
            ...
            thresh_rows = [
                ...
                [
                    "0.75 – 0.89",
                    "LLM-Assisted (High)",
                    "Good confidence. Verify evidence if needed.",
                ],
                [
                    "0.60 – 0.74",
                    "LLM-Assisted (Medium)",
