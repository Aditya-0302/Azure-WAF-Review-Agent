...
def _box(
    ...
    d.add(String(
        ...
        textAnchor="middle", fillColor=_C_TEXT,
    ))
    if sublabel:
        d.add(String(
            lx, y + h / 2 - fsize * 0.9, sublabel,
            fontSize=max(4.5, fsize - 1.5), fontName="Helvetica",
            textAnchor="middle", fillColor=_C_SUBTEXT,
        ))
...
def build_hierarchy_diagram(
    ...
    if not sub_nodes:
        ...
        d.add(String(
            ...
            fontSize=9, textAnchor="middle", fillColor=colors.grey,
        ))
        return d
    ...
    d.add(String(
        _SIDE_PAD, y_hdr_b + 0.33 * cm,
        "Azure Architecture Hierarchy  —  Subscription  →  Resource Group  →  Resource Type",
        fontSize=7.5, fontName="Helvetica-Bold", fillColor=_C_HDR_FG,
    ))
    d.add(String(
        width - _SIDE_PAD, y_hdr_b + 0.33 * cm,
        f"Assessment {str(agg.assessment_id)[:8]}…",
        fontSize=7, fontName="Helvetica", fillColor=_C_HDR_FG, textAnchor="end",
    ))
    ...
    x = x_start
    for rg, slot in zip(all_rgs, slots):
        cx = x + slot / 2
        rg_cx.append(cx)
        n = len(rg.resource_types)
        if n == 0:
    ...
    # RG → RT connectors
    if has_rts:
        for rg, cx, rt_cxs in zip(all_rgs, rg_cx, rg_rt_cx):
            if not rt_cxs:
                continue
    ...
    # Sub → RG connectors
    for sub, grp in zip(sub_nodes, sub_groups):
        if not grp:
            continue
    ...
    if has_rts:
        for rg, cx, rt_cxs in zip(all_rgs, rg_cx, rg_rt_cx):
            for rt_node, rtcx in zip(rg.resource_types, rt_cxs):
                _box(
                    d, rtcx - eff_rt / 2, y_rt_b, eff_rt, _RT_H,
    ...
    # ── RG nodes ───────────────────────────────────────────────────────────────
    for rg, cx in zip(all_rgs, rg_cx):
        _box(
            d, cx - eff_rg / 2, y_rg_b, eff_rg, _RG_H,
    ...
    for sub, grp in zip(sub_nodes, sub_groups):
        ...
        sub_cx = (min(rg_centers) + max(rg_centers)) / 2
        _box(
            d, sub_cx - _SUB_W / 2, y_sub_b, _SUB_W, _SUB_H,
