...
def _no_data(width: float, height: float, msg: str = "No data available") -> Drawing:
    ...
    d.add(
        Rect(2, 2, width - 4, height - 4, fillColor=_C_LGREY, strokeColor=_C_MGREY, strokeWidth=0.5)
    )
    d.add(
        String(
            width / 2,
...
def build_dashboard_data(
    ...
    try:
        ...
        hr_count = sum(
            1 for f in findings if any(c in _HUMAN_REVIEW_CODES for c in (f.waf_codes or []))
        )
        ...
        impact_counts: dict[str, dict[str, int]] = {
            cat: {s: 0 for s in _SEVERITY_ORDER} for cat in _IMPACT_COLORS
        }
        for f in findings:
    ...
    try:
        ...
        kpis = [
            (
                ...
                "0–100 composite",
                _score_color(data.overall_score),
            ),
            (
                "Compliance",
                f"{data.compliance_pct:.1f}%",
                "WAF posture",
                _score_color(data.compliance_pct),
            ),
            (
                "Risk Score",
            ...
            (
                "Critical / High",
                f"{data.critical_count} / {data.high_count}",
                "Immediate action",
                _C_CRIMSON if data.critical_count > 0 else _C_ORANGE,
            ),
            (
                "Medium",
                str(data.medium_count),
                "Within 30 days",
                _C_AMBER if data.medium_count > 0 else _C_GREEN,
            ),
        ...
        for idx, (title, value, subtitle, fill) in enumerate(kpis):
            ...
            d.add(
                Rect(
                    ...
                    card_w,
                    card_h,
                    fillColor=colors.HexColor("#A9A9A9"),
                    strokeColor=None,
                )
            )
            # Card face
...
def build_pillar_bars(
    ...
    try:
        ...
        for ref_pct, ref_col in [(70, _C_ORANGE), (90, _C_GREEN)]:
            ...
            d.add(Rect(rx, bar_bot, 0.6, bar_top - bar_bot, fillColor=ref_col, strokeColor=None))
            d.add(
                String(
                    rx,
                    bar_top + 0.05 * cm,
...
def build_severity_donut(
    ...
    try:
        ...
        active_sevs = [(s, c) for s, c in zip(_SEVERITY_ORDER, counts, strict=False) if c > 0]
        active_data = [c for _, c in active_sevs]
        active_colors = [_SEV_COLORS[s] for s, _ in active_sevs]
        [s.capitalize() for s, _ in active_sevs]
...
def build_radar_chart(
    ...
    try:
        ...
        for i in range(n):
            vx, vy = vertex(1.0, i)
            d.add(Line(cx, cy, vx, vy, strokeColor=_C_MGREY, strokeWidth=0.5))
        ...
        for i, sc in enumerate(scores):
            vx, vy = vertex(sc, i)
            d.add(Circle(vx, vy, 3.5, fillColor=_C_BLUE, strokeColor=_C_WHITE, strokeWidth=0.8))
...
def build_risk_heatmap_grid(
    ...
    try:
        ...
        pillars = [p for p in _PILLAR_ORDER if any(p in heatmap.get(s, {}) for s in severities)]
        ...
        for i, sev in enumerate(severities):
            ...
            d.add(
                String(
                    ...
                    fontSize=6.5,
                    textAnchor="end",
                    fillColor=_SEV_COLORS.get(sev, _C_DARK),
                )
            )
        ...
        # Legend
        for k, (_cnt_range, col, lbl) in enumerate(
            [
...
def build_trend_chart(
    ...
    try:
        ...
        for tick in [0, 20, 40, 60, 80, 100]:
            ...
            d.add(
                Rect(ox, ty, chart_w, 0.4, fillColor=colors.HexColor("#ECF0F1"), strokeColor=None)
            )
            d.add(
                String(
                    ox - 0.1 * cm,
        ...
        d.add(Rect(ox, ref_y, chart_w, 0.7, fillColor=colors.HexColor("#27AE60"), strokeColor=None))
        d.add(
            String(
                ox + chart_w + 0.1 * cm,
                ref_y - 3,
        ...
        # Line segments
        pts = [
            (ox + i * chart_w / max(n - 1, 1), oy + chart_h * min(scores[i], 100) / 100)
            for i in range(n)
        ]
        for i in range(len(pts) - 1):
            x1, y1 = pts[i]
            x2, y2 = pts[i + 1]
            d.add(Line(x1, y1, x2, y2, strokeColor=_C_BLUE, strokeWidth=2.0))

        # Dots and score labels
        for i, (px, py) in enumerate(pts):
            fill = _C_WHITE if i < len(pts) - 1 else _C_TEAL
            d.add(Circle(px, py, 3.5, fillColor=fill, strokeColor=_C_BLUE, strokeWidth=1.5))
            d.add(
                String(
                    px,
                    py + 5,
...
def build_business_impact_bars(
    ...
    try:
        ...
        for j, sev in enumerate(_SEVERITY_ORDER):
            lx = label_w + j * 2.2 * cm
            ly = height - 0.85 * cm
            d.add(Rect(lx, ly, 0.22 * cm, 0.16 * cm, fillColor=_SEV_COLORS[sev], strokeColor=None))
            d.add(
                String(
                    lx + 0.28 * cm,
                    ly + 0.01 * cm,
...
def build_coverage_visual(
    ...
    try:
        ...
        (
            ...
            if data.distinct_rules_assessed > 0
            else 0.0
        )
        tiles = [
            ("Resources Assessed", str(data.resources_assessed), "Total in scope", _C_BLUE),
            (
                "With Findings",
                str(data.resources_with_findings),
                "Have open issues",
                _C_RED if data.resources_with_findings > 0 else _C_GREEN,
            ),
            ("Clean Resources", str(no_findings), "No findings", _C_GREEN),
            (
                "Human Review Findings",
                str(data.human_review_findings),
                "Require expert review",
                _C_AMBER if data.human_review_findings > 0 else _C_TEAL,
            ),
        ]
            ...
            (
                ...
                [
                    ...
                    (
                        ...
                        "Satisfactory; improvement advised",
                    ),
