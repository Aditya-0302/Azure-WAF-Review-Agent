...
def build_severity_pie(
    ...
    active_labels = [s for s, c in zip(_SEVERITY_ORDER, counts) if c > 0]
    ...
    legend.colorNamePairs = [
        (c, f"{s.capitalize()}: {n}")
        for c, s, n in zip(active_colors, active_labels, pie.data)
    ]
    d.add(legend)
    return d
...
def build_resource_compliance_bar(
    ...
    pairs = sorted(zip(resource_types, compliance_pcts), key=lambda x: x[1])[:10]
...
def build_kpi_cards(
    ...
    for i, (label, value, fill) in enumerate(cards):
        ...
        y = pad_y
        # Shadow layer
        d.add(Rect(x + 0.05 * cm, y - 0.05 * cm, card_w, card_h,
                   fillColor=colors.HexColor("#BDC3C7"), strokeColor=None))
        # Card face
        d.add(Rect(x, y, card_w, card_h, fillColor=fill, strokeColor=None))
        # Value (large)
        d.add(String(
            x + card_w / 2, y + card_h * 0.42,
            str(value), fontSize=15, fontName="Helvetica-Bold",
            textAnchor="middle", fillColor=colors.white,
        ))
        # Label (small)
        d.add(String(
            x + card_w / 2, y + card_h * 0.13,
            label, fontSize=6.5, fontName="Helvetica",
            textAnchor="middle", fillColor=colors.white,
        ))
    return d
...
def build_risk_heatmap(
    ...
    for tick in [0, 25, 50, 75, 100]:
        ...
        d.add(String(tx, 0.05 * cm, str(tick),
    ...
    for i, (cat, score) in enumerate(zip(categories, scores)):
        ...
        # Category label
        d.add(String(label_w - 0.15 * cm, y + row_h * 0.28, cat,
                     fontSize=7, textAnchor="end",
                     fillColor=colors.HexColor("#2C3E50")))
        # Background track
        d.add(Rect(label_w, y, bar_w, row_h - 2 * gap,
                   fillColor=colors.HexColor("#ECF0F1"),
                   strokeColor=colors.HexColor("#BDC3C7"), strokeWidth=0.3))
        # Coloured fill
        if score >= 90:
        ...
        if fill_w > 0:
            d.add(Rect(label_w, y, fill_w, row_h - 2 * gap,
                       fillColor=fill, strokeColor=None))
        # Score percentage label
        d.add(String(score_x, y + row_h * 0.28, f"{score:.0f}%",
...
def build_resource_group_bar(
    ...
    pairs  = sorted(zip(rg_names, compliance_pcts), key=lambda x: x[1])[:12]
    ...
    for i, pct in enumerate(pcts):
        col = (
            _PASS_COLOR if pct >= 90
            else colors.HexColor("#F1C40F") if pct >= 50
            else _FAIL_COLOR
        )
        bar.bars[(0, i)].fillColor = col
...
def build_findings_by_pillar_stacked(
    ...
    d = Drawing(width, height)
    d.add(String(
        width / 2, height - 0.35 * cm,
        "Findings by Pillar (Stacked by Severity)",
        fontSize=8, fontName="Helvetica-Bold",
        textAnchor="middle", fillColor=colors.HexColor("#2C3E50"),
    ))
    ...
    for i, pillar in enumerate(pillars):
        ...
        d.add(String(
            label_w - 0.15 * cm, y + (row_h - gap) * 0.22,
            pillar.replace("_", " ").title()[:20],
            fontSize=6.5, textAnchor="end", fillColor=colors.HexColor("#2C3E50"),
        ))
        d.add(Rect(
            label_w, y, bar_area, row_h - 2 * gap,
            fillColor=colors.HexColor("#ECF0F1"),
            strokeColor=colors.HexColor("#BDC3C7"), strokeWidth=0.3,
        ))
        ...
        for sev in _SEV_ORDER_L:
            ...
            d.add(Rect(
                label_w + x_off, y, seg_w, row_h - 2 * gap,
                fillColor=_SEV_COLS_L[sev], strokeColor=None,
            ))
            x_off += seg_w

        d.add(String(
            count_x, y + (row_h - gap) * 0.22, str(total),
            fontSize=6.5, textAnchor="start", fillColor=colors.HexColor("#2C3E50"),
        ))

    leg_items = [("Critical", "critical"), ("High", "high"),
                 ("Medium", "medium"), ("Low", "low")]
    for j, (lbl, sev_key) in enumerate(leg_items):
        lx = label_w + j * 2.20 * cm
        ly = height - 0.80 * cm
        d.add(Rect(lx, ly, 0.22 * cm, 0.17 * cm,
                   fillColor=_SEV_COLS_L[sev_key], strokeColor=None))
...
def build_top_risk_contributors(
    ...
    for i, (label, score, worst_sev) in enumerate(items):
        ...
        d.add(String(
            label_w - 0.15 * cm, y + bar_h * 0.22,
            label[:30], fontSize=6, textAnchor="end",
            fillColor=colors.HexColor("#2C3E50"),
        ))
        d.add(Rect(
            label_w, y, bar_area, bar_h,
            fillColor=colors.HexColor("#ECF0F1"),
            strokeColor=colors.HexColor("#BDC3C7"), strokeWidth=0.3,
        ))
        ...
        d.add(String(
            score_x, y + bar_h * 0.22, f"{score:.0f}",
            fontSize=6, textAnchor="start", fillColor=colors.HexColor("#2C3E50"),
        ))
...
def build_compliance_breakdown(
    ...
    for ref_x, ref_lbl, ref_col in [
        ...
        d.add(Rect(ref_x, bar_bot, 0.6, bar_top - bar_bot,
                   fillColor=ref_col, strokeColor=None))
        d.add(String(ref_x, bar_top + 0.06 * cm, ref_lbl,
...
def build_compliance_roadmap(
    ...
    for tick in range(0, 101, 20):
        ...
        d.add(Rect(cx, ty, chart_w, 0.5,
                   fillColor=colors.HexColor("#ECF0F1"), strokeColor=None))
        d.add(String(cx - 0.12 * cm, ty - 3, f"{tick}%",
    ...
    d.add(Line(
        cx, ref_y, cx + chart_w, ref_y,
        strokeColor=colors.HexColor("#27AE60"),
        strokeWidth=1.0,
        strokeDashArray=[5, 3],
    ))
    d.add(String(
        cx + chart_w + 0.12 * cm, ref_y - 3,
        "90% target",
        fontSize=5.5, textAnchor="start",
    ...
    for i, (label, score) in enumerate(scenarios):
        ...
        # Track (background)
        d.add(Rect(bx, cy, bar_w, chart_h,
                   fillColor=colors.HexColor("#F2F3F4"),
                   strokeColor=colors.HexColor("#D5D8DC"), strokeWidth=0.3))
        # Filled portion
        if bh > 0:
...
def build_waf_benchmark_chart(
    ...
    for tick in [0, 25, 50, 75, 90, 100]:
        ...
        d.add(Line(tx, cy, tx, cy + chart_h,
                   strokeColor=colors.HexColor("#E8EAED"), strokeWidth=0.4))
        d.add(String(tx, cy - 0.32 * cm, f"{tick}%",
                     fontSize=5.5, textAnchor="middle",
                     fillColor=colors.HexColor("#7F8C8D")))
    ...
    for i, pillar in enumerate(reversed(pillars)):
        ...
        if fill_w > 0:
            d.add(Rect(cx, bar_y, fill_w, bar_h,
                       fillColor=bar_fill, strokeColor=None))

        # Target tick (dark-blue vertical line + small square marker)
        target_x = cx + chart_w * target / 100
        d.add(Line(target_x, bar_y, target_x, bar_y + bar_h,
                   strokeColor=colors.HexColor("#1A5276"), strokeWidth=2.0))
        marker_sz = 4
        d.add(Rect(
            target_x - marker_sz / 2,
            bar_y + (bar_h - marker_sz) / 2,
            marker_sz, marker_sz,
            fillColor=colors.HexColor("#1A5276"), strokeColor=None,
        ))

        # Pillar name label (left)
        d.add(String(
            cx - 0.15 * cm, bar_y + bar_h * 0.28,
        ...
        d.add(String(
            right_x, bar_y + bar_h * 0.55,
            f"{score:.1f}%",
            fontSize=7, fontName="Helvetica-Bold",
            textAnchor="start", fillColor=bar_fill,
        ))
        gap_str = f"+{gap_pp:.1f}pp" if gap_pp >= 0 else f"{gap_pp:.1f}pp"
        gap_col = (colors.HexColor("#27AE60") if gap_pp >= 0
                   else colors.HexColor("#E74C3C"))
        d.add(String(
            right_x, bar_y + bar_h * 0.05,
            gap_str,
            fontSize=6, textAnchor="start", fillColor=gap_col,
        ))
