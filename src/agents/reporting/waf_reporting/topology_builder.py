...
def build_topology(
    ...
    # Header bar
    d.add(Rect(0, canvas_h - _HEADER_H, canvas_w, _HEADER_H,
               fillColor=_COLOR_HEADER, strokeColor=None))
    d.add(String(
        _SIDE_PAD, canvas_h - _HEADER_H + 0.35 * cm,
        "Azure Resource Topology",
        fontSize=10, fontName="Helvetica-Bold", fillColor=_COLOR_TITLE,
    ))
    d.add(String(
        canvas_w - _SIDE_PAD, canvas_h - _HEADER_H + 0.35 * cm,
        f"Assessment {assessment_id[:8]}…  |  {subscription_count} subscription(s)",
        fontSize=8, fontName="Helvetica", fillColor=_COLOR_TITLE,
        textAnchor="end",
    ))
    ...
    for idx, stats in enumerate(items):
        ...
        # Box
        d.add(Rect(x, y, _BOX_W, _BOX_H,
                   fillColor=fill, strokeColor=_COLOR_BORDER, strokeWidth=0.5))

        # Resource type label (short name)
        short_name = _short_rt(stats.resource_type)
        d.add(String(
            x + _BOX_W / 2, y + _BOX_H - 0.55 * cm,
            short_name,
        ...
        # Metrics
        d.add(String(
            x + 0.15 * cm, y + _BOX_H - 1.05 * cm,
            f"Resources: {stats.total}",
            fontSize=6.5, fillColor=colors.HexColor("#34495E"),
        ))
        d.add(String(
            x + 0.15 * cm, y + _BOX_H - 1.50 * cm,
            f"Findings:  {stats.with_findings}",
            fontSize=6.5, fillColor=colors.HexColor("#34495E"),
        ))
        pct_str = f"Compliance: {stats.compliance_pct:.0f}%"
        d.add(String(
            x + 0.15 * cm, y + _BOX_H - 1.95 * cm,
            pct_str,
            fontSize=6.5, fillColor=colors.HexColor("#34495E"),
        ))
...
def _legend_box(
    ...
    size = 0.30 * cm
    d.add(Rect(x, y + 0.05 * cm, size, size,
               fillColor=fill, strokeColor=_COLOR_BORDER, strokeWidth=0.4))
    d.add(String(x + size + 0.10 * cm, y + 0.06 * cm, label,
